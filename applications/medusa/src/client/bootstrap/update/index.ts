...
export function registerUpdateListener (router: RouterInstance): void {
  timer.setInterval(
...
export async function refreshUpdateTimer (): Promise<void> {
...
function checkForUpdate (router: RouterInstance): void {
  if (PendingRequest) return

  PendingRequest = true

  // fetch version
  void axios.get('/api/version')
    .then(response => {
