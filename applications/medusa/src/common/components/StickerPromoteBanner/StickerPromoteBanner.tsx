...
export default function StickerPromoteBanner (): JSX.Element {
  ...
  const memoized = useMemo(() => new Random(hash(`${new Date()}`)), [])
  ...
  return (
    <Box p={2} position='relative' mt={8} bg='gray.900' borderRadius='md' w='100%'>
      <ExternalLink href={SHOP_LINK}>
        ...
        <Box borderRadius='md' opacity={0.1} position='absolute' top={0} bottom={0} left={0} right={0}
             bg='white'></Box>
        <Box position='relative' w='100%' h='100%'>
          ...
          <Text textShadow='1px 1px 2px #000' position='relative' fontWeight='md' color='gray.00' px={2} pt={16} pb={4}
                fontSize='lg'>
            <Text as='b'>
              Limited Edition </Text> <Text textShadow='' p={1} borderRadius='lg' bg='white' color='primary.500' as='b'>NSFW
            and
            SFW</Text> stickers from your favorite
            artists
          </Text>
