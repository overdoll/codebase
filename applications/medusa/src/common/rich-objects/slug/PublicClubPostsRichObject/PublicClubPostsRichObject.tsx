...
export default function PublicClubPostsRichObject ({
  ...
  const TITLE = `${TITLE_SEARCH_PREFIX} ${clubData.name}'s ${TITLE_FEATURES} - overdoll.com/${clubData.slug}`
  const DESCRIPTION = `${DESCRIPTION_PREFIX} ${clubData.name}'s ${DESCRIPTION_FEATURES} on overdoll.com. ${DESCRIPTION_CONTENT_DISCOVER} from ${clubData.name}.`
