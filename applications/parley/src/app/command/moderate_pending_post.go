...
func (h ModeratePendingPostHandler) Handle(ctx context.Context, moderatorId, pendingPostId, rejectionReasonId, notes string) error {
	...
	// has to be done asynchronously since it may perform some checks (i.e. creates a new user, etc..)
	if infractionAuditLog.IsDeniedWithInfraction() {
		...
		// Lock user account
		if _, err := h.eva.LockUser(ctx, infractionAuditLog.Contributor().ID(), infractionAuditLog.UserInfraction().Expiration()); err != nil {
