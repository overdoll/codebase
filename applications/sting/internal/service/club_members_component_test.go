...
func TestCreateClub_become_member_and_withdraw(t *testing.T) {
	...
	testingAccountId := "1pcKibRoqTAUgmOiNpGLIrztM9R"
	...
	err := client.Mutate(context.Background(), &becomeClubMember, map[string]interface{}{
		"input": types.BecomeClubMemberInput{ClubID: relay.ID(clb.ID)},
	})
	...
	env.ExecuteWorkflow(workflows.AddClubMember, clb.Reference, testingAccountId)
	...
	err = client.Mutate(context.Background(), &withdrawClubMembership, map[string]interface{}{
		"input": types.WithdrawClubMembershipInput{ClubID: relay.ID(clb.ID)},
	})
	...
	env.RegisterWorkflow(workflows.UpdateClubMemberTotalCount)
	// execute workflow manually since it won't be
	env.ExecuteWorkflow(workflows.RemoveClubMember, clb.Reference, testingAccountId)
	require.True(t, env.IsWorkflowCompleted())
	require.NoError(t, env.GetWorkflowError())
