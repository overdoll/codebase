...
func ParseElasticError(err error) error {
	...
	if e.Details != nil && e.Details.Reason != "" {
		...
		if e.Details.Type == "script_exception" {
			if len(e.Details.CausedBy) > 0 {
				return elastic.NewError(errors.New(fmt.Sprintf("elastic: Error %d (%s): %s [type=%s]", e.Status, http.StatusText(e.Status), e.Details.CausedBy["reason"], e.Details.Type)))
			}
		}
