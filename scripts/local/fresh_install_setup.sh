...
kubectl exec -it quick-es-default-0 -n elasticsearch -c elasticsearch -- curl -X PUT http://elastic:overdoll123@localhost:9200/_cluster/settings -H "Content-Type: application/json" -d '{ "persistent": { "action.auto_create_index": "false" } }'
